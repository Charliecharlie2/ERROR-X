//base by EMMYHENZ (ERROR-X Bot Inc.)
//YouTube:
//Instagram:
//Telegram:
//GitHub: @EMMY_HENZ-TECH
...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Emmy henz said you should Please type your WhatsApp number 😍\nFor example: +2349125042727 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')

         // Ask again when entering the wrong number
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +2349125042727")))
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
	        ...
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: EMMANUEL HENSHAW`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: EMMY_HENZ-TECH`))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: @DON DEREK `))
